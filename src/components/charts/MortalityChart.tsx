...
interface BuildingMortalityData {
  day: number
  mortality: number
}
...
export function MortalityChart({ standardData, buildingsData, data }: MortalityChartProps) {
      ...
      <div className="w-full h-full">
        <svg viewBox="0 0 400 200" className="w-full h-full">
          ...
          <polyline
            ...
            stroke="#ef4444"
            strokeWidth="2"
            points={data.map((d, i) => `${50 + i * 80},${180 - d.standard * 500}`).join(' ')}
          />
          ...
          <polyline
            ...
            stroke="#22c55e"
            strokeWidth="2"
            points={data.map((d, i) => `${50 + i * 80},${180 - d.actual * 500}`).join(' ')}
          />
  ...
  const allMortalityValues = [
    ...standardData.map(d => d.mortality),
    ...buildingsData.flatMap(building => building.data.map(d => d.mortality))
  ]
  const maxMortality = Math.max(...allMortalityValues, 10) // Match reference scale
  ...
  const scaleY = (mortality: number) => padding.top + chartHeight - (mortality / maxMortality) * chartHeight

  // Generate path string for a line
  const generatePath = (data: BuildingMortalityData[]) => {
    if (data.length === 0) return ""
    return data.map((d, i) => 
      `${i === 0 ? 'M' : 'L'} ${scaleX(d.day)} ${scaleY(d.mortality)}`
    ).join(' ')
  }
    ...
    <div className="w-full h-full">
      <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-full">
        <defs>
          ...
          <clipPath id="chart-clip-mortality">
            <rect 
              x={padding.left} 
              y={padding.top} 
              width={chartWidth} 
          ...
          <g key={day}>
            ...
            >
              {day}
                   ...
                   {/* Standard mortality line (Red) - SOLID LINE ONLY, NO DOTS, STAYS WITHIN BOUNDS */}
           {standardData.length > 0 && (
             <g clipPath="url(#chart-clip-mortality)">
        ...
        {/* Building mortality data - DOTS ONLY (no connecting lines) */}
        {buildingsData.map((building) => (
          <g key={building.id} clipPath="url(#chart-clip-mortality)">
            {/* Data points as dots only - no connecting lines */}
            {building.data.map((point, i) => (
              <circle
                key={i}
                cx={scaleX(point.day)}
                cy={scaleY(point.mortality)}
                r="6"
                fill={building.color}
                stroke="white"
                strokeWidth="3"

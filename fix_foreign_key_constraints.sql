...
SELECT '=== CURRENT CONSTRAINT ANALYSIS ===' as status;
...
SELECT '=== DROPPING INCORRECT CONSTRAINT ===' as status;
...
BEGIN
    ...
    IF EXISTS (
        ...
        RAISE NOTICE 'Dropped incorrect participants_farm_id_fkey constraint';
    ELSE
        RAISE NOTICE 'participants_farm_id_fkey constraint not found';
    END IF;
END $$;

-- 3. Check if there are any orphaned records that need to be fixed first
...
-- 4. Fix orphaned records by creating farms for users if needed
...
BEGIN
    ...
    LOOP
        ...
        SELECT id INTO user_id FROM auth.users WHERE id = participant_record.farm_id;
        ...
        IF user_id IS NOT NULL THEN
            ...
            RAISE NOTICE 'Fixed participant % (%), assigned to farm %', 
                participant_record.name, participant_record.id, correct_farm_id;
        ELSE
...
SELECT '=== ADDING CORRECT FOREIGN KEY CONSTRAINT ===' as status;
...
BEGIN
    ...
    RAISE NOTICE 'Added correct participants_farm_id_fkey constraint';
EXCEPTION
...
-- Check that all participants now have valid farm_id references
